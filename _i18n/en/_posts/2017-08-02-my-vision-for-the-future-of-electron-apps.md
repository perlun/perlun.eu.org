...
Also, it's not necessarily so that _today's_ generation of NPM packages (authored in JavaScript or TypeScript) would necessarily be compiled to WebAssembly bytecode. What I'm personally hoping for would be a trend where certain packages are being reimplemented in C, C++ or similar (ideally, Rust). The ABI would be preserved so that you can just change your `package.json` to refer to `lodash_wasm` instead of `lodash`, and you will get the WebAssembly-optimized version. One incredibly nice thing with this is that should also let each package provide _one_ single `.wasm` file with the compiled binary version of the package. This will be an incredibly nice improvement over today's situation where a regular Node.js-based application can consist of literally 10 000 of files in the `node_modules` folder.
